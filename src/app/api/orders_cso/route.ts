...
export async function POST(request: Request) {
  try {
    ...
    const roCount = parseInteger(formData.get('ro_count'), 0) || 0;
    ...
    const orderResult = await withMysqlTransaction(async (connection) => {
      ...
      await insertActivityLog(connection, {
        ...
        target: 'Pesanan',
        details: `Order: ${uniqueCode} | Source: CSO_AUTO | Status Awal: pending | Pesanan dibuat`,
        ipAddress,
      });
    ...
    console.error('Error creating CSO order:', error);
