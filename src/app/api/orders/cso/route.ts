...
export async function POST(request: Request) {
  try {
    ...
    const customerIdStr = String(formData.get('customer_id') || '').trim();
    ...
    const warehouseIdParsed = parseInteger(formData.get('warehouse_id'));
    const warehouseId = warehouseIdParsed > 0 ? warehouseIdParsed : null;
    const courierName = String(formData.get('courier_name') || '').trim();
    const paymentMethod = String(formData.get('payment_method') || '').trim();
    const noPaymentMethodId = parseInteger(formData.get('no_payment_method_id'));
    ...
    const paymentAccountId = parseInteger(formData.get('payment_account_id'));

    if (!customerName || !whatsapp || !address || !subdistrict) {
      return NextResponse.json(
        { status: 'error', message: 'Data pelanggan belum lengkap.' },
        { status: 400 },
      );
    }
    ...
    if (scalevOrderId && scalevOrderId.length !== 13) {
      return NextResponse.json(
        { status: 'error', message: 'Kode Scalev harus tepat 13 karakter' },
        { status: 400 },
      );
    }
    ...
    if (pIds.length === 0) {
      return NextResponse.json(
        { status: 'error', message: 'Keranjang pesanan masih kosong.' },
        { status: 400 },
      );
    }
    ...
    const orderResult = await withMysqlTransaction(async (connection) => {
      ...
      const customer = await upsertCustomer(connection, {
        customerId: parseInteger(customerIdStr, 0) || null,
        customerName,
        whatsapp,
        email: email || null,
        address,
      ...
      const uniqueCode = await generateUniqueCode(connection, 'AKU');
      ...
      const biayaLainnya = manualFeeCod + otherFee;
      ...
      const [insertOrderResult] = await connection.query<ResultSetHeader>(
        `
          INSERT INTO orders (
            ...
            additional_shipping_cost, shipping_discount, created_by
          ) VALUES (
            ?, ?, ?, ?,
            ?, ?, ?,
            ?, ?, ?, ?, ?,
      ...
      await insertActivityLog(connection, {
        ...
        target: 'Pesanan',
        details: `Order: ${uniqueCode} | Source: AKUISISI | Status Awal: pending | Pesanan dibuat`,
        ipAddress,
      });
    ...
    return NextResponse.json({
      ...
      order_code: orderResult.unique_code,
      order_id: orderResult.id,
    });
  } catch (error: unknown) {
    console.error('Error creating akuisisi order:', error);

    const message = error instanceof Error ? error.message : 'Internal server error';
    const statusCode =
      message === 'Kode Scalev sudah digunakan.' || message === 'Data pelanggan belum lengkap.'
        ? 400
        : 500;
